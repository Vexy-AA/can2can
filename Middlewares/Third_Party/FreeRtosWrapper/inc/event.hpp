...
namespace OsWrapper
{
  ...
  public:
    ...
    __forceinline void Signal(tEventBits maskBits)
    {
      mask = maskBits;
      RtosWrapper::wSignalEvent(handle, mask);
    }

    __forceinline tEventBits Wait(const tEventBits maskBits) const
    {
      tEventBits setBits = RtosWrapper::wWaitEvent(handle, maskBits, timeOut, EventMode::waitAllBits);
      if (setBits & maskBits)
        return setBits;
      else
        return 0;
